...
Our \textbf{Proposed Model} processes raw video streams by first sampling video frames and extracting aligned 3-channel RGB face crops $X \in \mathbb{R}^{3 \times 224 \times 224}$ using MTCNN. These 3-channel RGB frames are processed through two parallel feature extraction streams, as illustrated in Fig.~\ref{fig:arch}.

\subsection{Stream 1: Semantic Visual Branch (CLIP ViT-B/32)}
Stream 1 feeds the 3-channel RGB face crop $X$ directly into OpenAI's pre-trained \textbf{CLIP ViT-B/32} Vision-Language Transformer to extract high-level visual semantics. We freeze the bottom 10 Vision Transformer blocks and unfreeze only the top 2 blocks:
\begin{equation}
...
\end{equation}
This produces 3 noise residual channels $R = [R_1 \Vert R_2 \Vert R_3] \in \mathbb{R}^{3 \times 224 \times 224}$, which are passed into a pre-trained \textbf{EfficientNet-B0} backbone with Squeeze-and-Excitation (SE) attention:
\begin{equation}
